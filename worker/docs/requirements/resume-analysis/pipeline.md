...
## 7. 표류 방어 — 골든 샘플 픽스처 테스트

크로스 레포라 Worker CI·리뷰는 백엔드를 못 본다. 계약 사본이 알아채지 못한 채 어긋나는 것을 **테스트로 고정**한다.

- 실제 스트림 필드맵 예시(JSON)를 워커 repo에 **골든 샘플**로 커밋.
- 테스트: **요청 decode**(문자열 필드맵 → pydantic 모델) + **상태 encode**(모델 → 문자열 필드맵) **양방향**을
  골든 샘플과 정확히 대조. 계약이 바뀌면 픽스처가 바뀌고, 그건 PR에 눈에 보인다.
- MVP: 골든 샘플은 백엔드 record를 보고 손으로 작성해 계약 고정점으로 삼는다.
