...
부하 리허설(2026-09-02)에서 결함 발견: 요청마다 새 PG 연결을 열어 동시 수백 요청 시
`FATAL: too many clients`(PG 기본 상한 100 초과)가 발생했다. 수정 후 구조:

- **HTTP 경로는 커넥션 풀 전용 사용** — 요청은 풀에서 연결을 빌려 처리 전 구간(파이프라인 + 상태
  재조회)에 쓰고 반납한다. 풀이 만드는 연결은 최대 `sync_pool_max_size`(기본 10)개이므로, 유입량과
  무관하게 **총 PG 연결 = max_size + 스트림 경로 1~2개**로 유한하다 — too many clients 구조적 불가
- **초과 요청은 대기** — 연결이 모두 대출 중이면 반납을 기다리고, `sync_pool_wait_timeout_s`(기본 60s)
  초과 시 **503**으로 거절한다(Spring은 비-2xx → FAILED 전이). 대기 + 처리 시간이 Spring read
  timeout(120s) 안에 들도록 타임아웃을 정한다
- **스트림 소비 경로는 풀을 쓰지 않는다** — 기존 요청당 fresh 연결 유지(순차 소비라 연결 ≤1).
  HTTP 부하가 풀을 다 차지해도 스트림 소비가 연결 대기에 빠지지 않는다
- **처리량 상한 ≈ max_size ÷ 건당 처리 시간** — max_size가 동기 모드의 처리량 조절 노브다.
  단, 실효 동시 추론은 **min(max_size, to_thread 스레드풀 크기 min(32, cpu+4))** — 추론(fake sleep /
  실제 Bedrock sync 호출)이 공용 스레드풀의 스레드를 점유하므로, max_size를 스레드풀 크기 이상으로
  올려도 처리량은 늘지 않는다. **측정 시 max_size는 스레드풀 크기 이하로**(로컬 10코어 기준 ≤14).
  스레드풀을 키우지 않는 이유: 스레드 상한은 실배포에서 동시 Bedrock 호출을 막는 사실상의 보호
  장치라, 부하 테스트 편의로 실서비스 용량 특성을 바꾸지 않는다
