...
- **커넥션 풀을 두 경로(HTTP·스트림)가 같이 쓴다** — 모든 DB 접근은 풀에서 커넥션을 가져와
  쓰고 반환한다. 풀이 만드는 커넥션은 최대 `db_pool_max_size`(기본 10)개라, 요청이 아무리
  몰려도 **워커가 여는 PG 커넥션은 풀 크기를 넘지 않는다** — too many clients 가 아예 날 수 없고,
  설정도 한 곳에서 관리된다
- **스트림 소비는 `stream_max_workers`(기본 1)로 N건을 동시에 처리** — FastStream 구독자의
  `max_workers` 를 사용한다. 동시에 처리해도 ACK 는 메시지별로 핸들러가 끝난 뒤에 하므로,
  실패한 메시지가 PEL 에 남아 회수되는(§3) 설계는 그대로다. 같은 resumeId 가 동시에 처리되는
  경우는 CAS 상태 전이(§3.3)가 걸러낸다. 회수 구독자는 지금처럼 한 건씩 처리한다
  (복구는 드물어 동시에 처리할 이유가 없음)
- **N ≤ P 기동 검증** — `stream_max_workers` 가 `db_pool_max_size` 보다 크면 설정 오류로 기동이
  실패한다(fail-fast). 동시 작업이 풀보다 많으면 커넥션을 기다리느라 동시 처리 수가 조용히
  풀 크기로 깎이기 때문
- **커넥션을 못 가져온 요청은 이렇게 된다** — HTTP: `db_pool_wait_timeout_s`(기본 60s)까지
  기다리다가 **503** (Spring 은 비-2xx 를 받아 FAILED 로 바꾼다. 대기 + 처리 시간이 Spring read
  timeout 120s 안에 들도록 유지). 스트림: 같은 타임아웃 뒤 예외 → ACK 없음 → PEL 회수로 복구
- **처리량 상한 ≈ 동시 처리 수 ÷ 건당 처리 시간** — 동시 처리 수를 정하는 설정은 HTTP 가
  `db_pool_max_size`, 스트림이 `stream_max_workers`. 단, 실제로 동시에 도는 추론은 **min(동시
  작업 수, to_thread 스레드풀 min(32, cpu+4))** 까지다 — 추론(fake sleep / 실제 Bedrock sync 호출)이
  공용 스레드풀의 스레드를 잡고 있으므로 그 이상 올려도 처리량은 늘지 않는다(로컬 10코어 기준
  ≤14 권장). 스레드풀을 키우지 않는 이유: 스레드 상한이 실배포에서 동시 Bedrock 호출이 몰리는
  것을 막아주고 있어서
- **공정하게 비교하려면** — 동기/비동기 모드의 워커 동시 처리 수를 같은 값으로 맞춰 측정한다
  (`stream_max_workers` = 동기 동시 처리 수). 다르면 디스패치 방식이 아니라 워커 동시성을
  비교하는 실험이 된다
...
- 2026-09-02 완료 처리량(분당 ~5건)을 올리기 위해 스트림 소비가 `stream_max_workers` 로 여러 건을
