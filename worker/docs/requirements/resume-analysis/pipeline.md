...
부하 리허설(2026-09-02)에서 두 문제가 확인됐다: (1) HTTP 경로가 요청마다 새 PG 연결을 열어
동시 수백 요청 시 `FATAL: too many clients`(PG 기본 상한 100 초과), (2) 스트림 소비가 순차
(동시성 1)라 완료 처리량이 분당 ~5건에 그침. 수정 후 구조:

- **커넥션 풀을 두 경로(HTTP·스트림)가 공유한다** — 모든 DB 접근은 풀에서 연결을 빌려 쓰고
  반납한다. 풀이 만드는 연결은 최대 `db_pool_max_size`(기본 10)개이므로, 유입량과 무관하게
  **워커의 PG 연결 총량 = 풀 크기 하나로 묶인다** — too many clients 구조적 불가, 설정 관리 일원화
- **스트림 소비는 `stream_max_workers`(기본 1)로 동시 N건 처리** — FastStream 구독자의
  `max_workers` 를 사용한다. ACK 는 동시 처리에서도 메시지별·핸들러 완료 후이므로 실패 시
  PEL 잔류 → 회수(§3) 설계가 그대로 유지된다. 같은 resumeId 경쟁은 CAS 상태 전이(§3.3)가 처리.
  회수 구독자는 순차 유지(복구는 드물어 동시화 이득 없음)
- **N ≤ P 기동 검증** — `stream_max_workers` > `db_pool_max_size` 면 설정 오류로 기동 실패
  (fail-fast). 스트림 동시 작업이 풀보다 많으면 연결 대기로 동시성이 조용히 P 로 깎이기 때문
- **연결을 못 빌린 요청의 동작** — HTTP: `db_pool_wait_timeout_s`(기본 60s) 대기 후 **503**
  (Spring 은 비-2xx → FAILED 전이. 대기 + 처리 < Spring read timeout 120s 유지). 스트림: 같은
  타임아웃 후 예외 → ACK 없음 → PEL 회수로 자연 복구
- **처리량 상한 ≈ 동시성 ÷ 건당 처리 시간** — 동시성 노브는 HTTP = `db_pool_max_size`,
  스트림 = `stream_max_workers`. 단, 실효 동시 추론은 **min(동시 작업 수, to_thread 스레드풀
  min(32, cpu+4))** — 추론(fake sleep / 실제 Bedrock sync 호출)이 공용 스레드풀을 점유하므로
  그 이상 올려도 처리량은 늘지 않는다(로컬 10코어 기준 ≤14 권장). 스레드풀을 키우지 않는 이유:
  스레드 상한은 실배포에서 동시 Bedrock 호출을 막는 사실상의 보호 장치
- **공정 비교 조건** — 동기/비동기 암은 워커 동시성을 같게 맞춰 측정한다
  (`stream_max_workers` = 동기 동시성 = 같은 값). 다르면 디스패치 방식이 아니라 워커 동시성을
  비교하는 실험이 된다
...
- 2026-09-01 동기 HTTP 디스패치 실험(HBB1-327): 이력서 워커를 AsgiFastStream으로 전환해 스트림 소비와 같은
  ...
  제거 가능. → §11.
- 2026-09-02 부하 리허설에서 동기 엔드포인트가 요청마다 새 PG 연결을 열어 too many clients 발생(1,195회) →
  HTTP 경로 전용 커넥션 풀 도입(`sync_pool_max_size`, 대기 초과 503). 스트림 경로는 불변. 스레드풀은
  의도적으로 유지(실배포 Bedrock 동시 호출 방벽). → §11.4.
- 2026-09-02 완료 처리량(분당 ~5건)을 올리기 위해 스트림 소비를 `stream_max_workers` 로 동시화하고,
  풀을 두 경로 공유로 전환(설정 이름 `sync_pool_*` → `db_pool_*`). 인스턴스 증설 대신 프로세스 내
  동시성으로 확장. N ≤ P 기동 검증. 두 암의 워커 동시성을 같게 맞춰야 통제된 비교가 됨. → §11.4.
