...
핵심은 **상태 발행이 Spring 이 구독하는 Pub/Sub 채널에 JSON 으로 나가는지** 확인하는 것.
...
@pytest.mark.asyncio
async def test_상태발행은_PubSub_채널에_JSON으로():
    fake = _FakeRedis()
    await publish_status(fake, resume_id=5, user_id=3, status=AnalysisStatus.EMBEDDING)

    assert len(fake.published) == 1
    channel, message = fake.published[0]
    assert channel == StatusChanged.CHANNEL
    assert isinstance(message, str)  # JSON 문자열 하나 — Spring 이 그대로 파싱한다
    # 키·값은 encode() 와 같고 전부 문자열
...
async def test_상태발행_message_기본은_빈문자열():
    ...
    assert json.loads(fake.published[0][1])["message"] == ""
