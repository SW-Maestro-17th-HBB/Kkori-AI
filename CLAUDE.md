...
- **이력서 워커는 AsgiFastStream(구독자 + HTTP 단일 프로세스)** — 동기 디스패치 실험(HBB1-327, worker PRD §11)용 `POST /internal/analyses/resume` 를 스트림 소비와 같은 프로세스로 서빙한다(FastStream 0.7 내장 ASGI + uvicorn, 기본 127.0.0.1:8000 = Spring 로컬 기본 base URL, Docker 는 `--host 0.0.0.0` + EXPOSE 8000). FastAPI 미도입 — 고정 경로 1개에 프레임워크 추가는 과함. 스트림 경로와 같은 배선(`_process`)을 태워 상태 이벤트까지 작업량이 동일하고, 2xx 는 처리 후 상태 재조회로 EMBEDDED 일 때만. fake 지연은 `KKORI_WORKER_FAKE_DELAY_SECONDS`(분석 1건당 총 ≈N, `time.sleep` — to_thread 경유라 루프 안 막힘). HTTP 경로의 DB 접근은 전용 커넥션 풀(`sync_pool_max_size`, 기본 10 — 부하 리허설에서 요청당 새 연결이 too many clients 를 일으켜 도입, 대기 초과는 503)이고 스트림 경로는 기존 요청당 연결 유지. 리포트 워커(`src.report.main:app`)는 FastStream 유지. 실험 종료 후 제거 가능
